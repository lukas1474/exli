...
export const calculatePopulation = (): void => {
  ...
  const sumCountry: number = euFilterCountriesNoA.reduce(
    (a, b) => a + b.population,
    0
  );
  ...
  const message = (sumCountry > population) ? 
    `The total population of the countries in array is greater than 500 milion and amounts to: ${sumCountry} people`
    :
    `The total population of the countries in array is less than 500 milion and amounts to: ${sumCountry} people`

  console.log("EU", euFilterCountries);
  console.log("EU bez a", euFilterCountriesNoA);
  console.log("suma", sumCountry);
  console.log(message);
};
